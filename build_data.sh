...
do
	...
	cp -f mols/$path/b3lyp/*.gjf $geoms_out
	...
	do
		python fileparser.py -L -f mols/$path/$subpath | tail -n+2 | sed -e '/^$/d' > $main/$path/$subpath.csv
		python - <<EOF
import csv
import operator

with open("$main/$path/$subpath.csv", "r") as f, open("$main/$path/$subpath.txt", "w") as f2:
