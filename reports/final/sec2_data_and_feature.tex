...
\begin{flalign*}
\bold{H_{pq}^{DFT}(C)} &= \bold{H_{pq}^{DFT, core}} + \sum_{rs}{(pq|rs)\boldsymbol{\rho_{rs}}} + \bold{H_{pq}^{DFT,xc}} \\[2ex]
\text{where:  } &\boldsymbol{\rho_{rs}} = \sum_{i=1}^m{\bold{C_{ri}^*C_{si}}}
\end{flalign*}
...
\begin{flalign*}
\boldsymbol{\epsilon_i} &= \boldsymbol{\epsilon_i^{(0)}} + \boldsymbol{\epsilon_i^{(1)}} + \boldsymbol{\epsilon_i^{(2)}} + \cdots \\
\end{flalign*}

Where: 
\begin{flalign*}
\boldsymbol{\epsilon_i^{(0)}} &= \bold{E_i} \\
\boldsymbol{\epsilon_i^{(1)}} &= <\Phi_i|H^{DFT} - H^{SEQC}|\Phi_i> \\
\boldsymbol{\epsilon_i^{(2)}} &= \sum_{j\ne i}{\frac{|<\Phi_i|H^{DFT} - H^{SEQC}|\Phi_j>|^2}{\bold{E_i} - \bold{E_j}}} \\
\end{flalign*}
